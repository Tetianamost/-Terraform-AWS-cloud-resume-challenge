...
resource "aws_lambda_function" "resume_website" {
  s3_bucket     = "my-resume-web-lambda"
  s3_key        = "lambda.zip"
  ...
  "Statement": [
    {
      ...
      "Action": "s3:*",
      "Resource": [
        "arn:aws:s3:::my-resume-web-lambda",
        "arn:aws:s3:::my-resume-web-lambda/*"
      ]
    }
  ]
...
resource "aws_iam_role" "lambda_role" {
  name               = "my-resume-web-lambda-role"
...
resource "aws_iam_policy" "lambda_dynamodb_policy" {
  name   = "my-resume-web-lambda-dynamodb-policy"
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_api_gateway_rest_api" "resume-web" {
...
resource "aws_api_gateway_method" "resume-web" {
  rest_api_id   = aws_api_gateway_rest_api.resume-web.id
  resource_id   = aws_api_gateway_rest_api.resume-web.root_resource_id
  http_method   = "ANY"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "resume-web" {
  rest_api_id = aws_api_gateway_rest_api.resume-web.id
  resource_id = aws_api_gateway_rest_api.resume-web.root_resource_id
  http_method = aws_api_gateway_method.resume-web.http_method
...
resource "aws_lambda_permission" "apigw" {
  ...
  source_arn = "arn:aws:execute-api:us-east-1:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.resume-web.id}/*/*"
}
resource "aws_api_gateway_method_response" "resume-web" {

  depends_on      = [aws_api_gateway_method.resume-web]
  rest_api_id     = aws_api_gateway_rest_api.resume-web.id
  resource_id     = aws_api_gateway_rest_api.resume-web.root_resource_id
  http_method     = aws_api_gateway_method.resume-web.http_method
  status_code     = 200
...
resource "aws_api_gateway_integration_response" "resume-web" {
  rest_api_id = aws_api_gateway_rest_api.resume-web.id
  resource_id = aws_api_gateway_rest_api.resume-web.root_resource_id
  http_method = aws_api_gateway_method.resume-web.http_method
  status_code = "200"
...
resource "aws_api_gateway_deployment" "resume-web" {
  depends_on  = [aws_api_gateway_integration_response.resume-web, aws_api_gateway_integration.resume-web]
  rest_api_id = aws_api_gateway_rest_api.resume-web.id
  stage_name  = "dev1"
}
